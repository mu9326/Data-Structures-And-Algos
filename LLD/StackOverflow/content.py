        ...
        with self._lock:
            ...
            if isinstance(self, Question):
                event_type = (
                    ...
                    else EventType.DOWNVOTE_QUESTION
                )
            else:
                event_type = (
                    EventType.UPVOTE_ANSWER
                    if vote_type == VoteType.UPVOTE
                    else EventType.DOWNVOTE_ANSWER
                )
...
class Question(Post):
    ...
    def accept_answer(self, answer: "Answer"):
        with self._lock:
            ...
            if (
                self.author.get_id() != answer.get_author().get_id()
                and self.accepted_answer is None
            ):
                ...
                self.notify_observers(
                    Event(EventType.ACCEPT_ANSWER, answer.get_author(), answer)
                )
